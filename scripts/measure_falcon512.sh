...
export TIMING_RESULTS_DIR=results/falcon512_times
export MEMORY_RESULTS_DIR=results/falcon512_memory
export GENERATED_DIR=results/falcon512_generated

# Commands used to generate keys, CSRs, and certificates using OpenSSL via oqs-provider
export GEN_CMD="./openssl genpkey -provider default -provider oqsprovider -algorithm falcon512 -out $GENERATED_DIR/private.key"
...
export CSR_CMD="./openssl req -new -key $GENERATED_DIR/private.key -out $GENERATED_DIR/request.csr -subj \"/C=US/ST=Georgia/L=Atlanta/O=My Company Inc/CN=falcon.example.com\" -provider default -provider oqsprovider"
