...
export MEMORY_RESULTS_DIR=results/mayo2_memory
export GENERATED_DIR=results/mayo2_generated

# Commands used to generate keys, CSRs, and certificates using OpenSSL via oqs-provider
export GEN_CMD="./openssl genpkey -provider default -provider oqsprovider -algorithm mayo2 -out $GENERATED_DIR/private.key"
...
export CSR_CMD="./openssl req -new -key $GENERATED_DIR/private.key -out $GENERATED_DIR/request.csr -subj \"/C=US/ST=Georgia/L=Atlanta/O=My Company Inc/CN=mayo2.example.com\" -provider default -provider oqsprovider"
