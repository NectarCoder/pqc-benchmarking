...
# ATTENTION: This script has been modified from the original fullbuild.sh given in oqs-provider.
# Run the general setup-benchmarking-env.sh script to automatically pull the oqs-provider repo and
# overwrite that script with this one.
...
# Detect available build system and C compiler early to provide clearer errors
if command -v ninja >/dev/null 2>&1; then
   CMAKE_GENERATOR_NAME="Ninja"
   BUILD_TOOL="ninja"
else
   # Fall back to Unix Makefiles if ninja isn't installed
   CMAKE_GENERATOR_NAME="Unix Makefiles"
   BUILD_TOOL="make"
fi

# Check for a C compiler (cc/gcc/clang). Fail early with a helpful message.
if ! command -v cc >/dev/null 2>&1 && ! command -v gcc >/dev/null 2>&1 && ! command -v clang >/dev/null 2>&1; then
   echo "No C compiler found (gcc/clang). Please install a C compiler (e.g. build-essential on Debian/Ubuntu) and re-run."
   exit 1
fi

# Normalize to the project root (script is in oqs-provider/scripts)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJ_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$PROJ_ROOT" || { echo "Failed to cd to project root: $PROJ_ROOT"; exit 1; }
...
if [ -z $liboqs_DIR ]; then
 if [ ! -f ".local/lib/liboqs.$STATLIBEXT" ]; then
  ...
  #    All: include all algorithms supported by liboqs (default)
   echo "Using CMake generator: $CMAKE_GENERATOR_NAME (build tool: $BUILD_TOOL)"
   # Build liboqs in its directory and return explicitly to the project root
   cd liboqs && cmake -G "$CMAKE_GENERATOR_NAME" $CMAKE_PARAMS $DOQS_ALGS_ENABLED $CMAKE_OPENSSL_LOCATION $DOQS_LIBJADE_BUILD -DCMAKE_INSTALL_PREFIX=$(pwd)/../.local -S . -B _build && cmake --build _build --target install -- $MAKE_PARAMS && cd "$PROJ_ROOT"
  if [ $? -ne 0 ]; then
