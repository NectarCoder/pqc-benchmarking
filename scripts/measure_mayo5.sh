...
export TIMING_RESULTS_DIR=results/mayo5_times
export MEMORY_RESULTS_DIR=results/mayo5_memory
export GENERATED_DIR=results/mayo5_generated

# Commands used to generate keys, CSRs, and certificates using OpenSSL via oqs-provider
export GEN_CMD="./openssl genpkey -provider default -provider oqsprovider -algorithm mayo5 -out $GENERATED_DIR/private.key"
...
export CSR_CMD="./openssl req -new -key $GENERATED_DIR/private.key -out $GENERATED_DIR/request.csr -subj \"/C=US/ST=Georgia/L=Atlanta/O=My Company Inc/CN=mayo5.example.com\" -provider default -provider oqsprovider"
